    ...
    override fun onMapReady(mapboxMap: MapboxMap) {
        mapboxMap.setStyle(Style.MAPBOX_STREETS) {
            ...
            padding_seekbar.apply {
                max = SEEKBAR_MAX / SEEKBAR_STEP
                progress = (40).also {
                    seekBarProgress = it
                    padding_textview.text = String.format(
                        getString(R.string.alternative_route_click_padding),
                        it
                    )
                }
                setOnSeekBarChangeListener(
                    object : SeekBar.OnSeekBarChangeListener {
                        ...
                        override fun onStopTrackingTouch(seek: SeekBar) {
                            navigationMapboxMap?.updateClickDistancePadding(seekBarProgress)
                        }
                    }
                )
            }
            ...
            navigationMapboxMap = NavigationMapboxMap.Builder(
                ...
                mapboxMap,
                this
            ).withRouteClickPadding(seekBarProgress).build()
